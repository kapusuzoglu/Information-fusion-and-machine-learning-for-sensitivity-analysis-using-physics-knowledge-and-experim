...
max_iter = 1000,
filename1="Np_GP2_poro_upd.py",
filename2="Np_GP2_poro_upd.py",
pre_train_hyp,
nobs = 10
)
{
	source_python(filename1)
    model <- pass_arg_upd(nsim, nobs, pre_train_hyp)
    if(sequential){
...
if(MCmethod!="sobol2007"){
	...
	# ysimu <- simulate(object = model, nsim = nsim,  newdata=X, 
                            # cond=TRUE, checkNames=FALSE, max_iter=1000,type)
	source_python(filename2)
    ysimu <- pass_arg2(X, nsim, model)
