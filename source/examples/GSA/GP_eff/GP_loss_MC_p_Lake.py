...
def pass_arg(nsim, tr_size):
    ...
    uX1 = uX1[pre_tr_size:pre_tr_size+loss_num,:-1]
    uX2 = uX2[pre_tr_size:pre_tr_size+loss_num,:-1]
    uY1 = uX1[pre_tr_size:pre_tr_size+loss_num,-1:]
    uY2 = uX2[pre_tr_size:pre_tr_size+loss_num,-1:]
    ...
    def obj_funct(theta, eval_gradient=True):
        if eval_gradient:
            lml, grad = log_marginal_likeli(gp,theta, eval_gradient=True, clone_kernel=False)
            return -lml, -grad
        else:
            return -log_marginal_likeli(gp, theta, clone_kernel=False)
    ...
    kernel = C(5.0, (0.5, 1e1)) * RBF(length_scale = [1] * trainX.shape[1], length_scale_bounds=(1e-1, 1e9))
    #kernel = C(5.0, (1e-2, 1e3)) * RBF(length_scale = [1] * trainX.shape[1], length_scale_bounds=(1e-3, 1e4))
    gp = GaussianProcessRegressor(optimizer = custom_optimizer, kernel=kernel, alpha =.1, n_restarts_optimizer=10)
    ...
    return [gp, max_in_column_Xc, min_in_column_Xc]